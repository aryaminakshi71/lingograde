// Button Component - Based on existing shared-ui patterns
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
   ({className, variant, size, asChild = false, loading, isLoading, leftIcon, rightIcon, children, disabled, 'aria-label': ariaLabel, ...props}, ref) => {
     const Comp = asChild ? Slot : 'button'
     const isIconOnly = !children && (leftIcon || rightIcon)
     const computedAriaLabel = isIconOnly ? ariaLabel || (leftIcon ? 'Action' : 'Action') : ariaLabel
     // Use isLoading if loading is not provided (backward compatibility)
     const isActuallyLoading = loading ?? isLoading ?? false
     
     // Filter out isLoading from props to prevent it from being passed to DOM
     const { isLoading: _, ...domProps } = props as any
     ...
     return (
       <Comp
         ...
         aria-disabled={disabled || isActuallyLoading ? 'true' : undefined}
